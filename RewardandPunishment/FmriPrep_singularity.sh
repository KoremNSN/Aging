#!/bin/bash
#
#SBATCH --j Aging_fp_21
#SBATCH --array=1 #-47 # Replace indices with the right number of subjects
...
SINGULARITY_CMD="singularity run --cleanenv -B $HOME/.cache/templateflow:/templateflow -B ${WORK_DIR}:/work /gpfs/gibbs/pi/levy_ifat/shared/fmriPrep/fmriprep-21.0.0.simg"


echo Starting ${SUBJ[$SLURM_ARRAY_TASK_ID-1]}

cmd="${SINGULARITY_CMD} ${BIDS_DIR} ${BIDS_DIR}/${DERIVS_DIR} participant --skip_bids_validation --participant-label ${SUBJ[$SLURM_ARRAY_TASK_ID-1]} -w /work/ -vv --omp-nthreads 8 --nthreads 12 --mem_mb 30000 --output-spaces MNI152NLin2009cAsym anat fsnative fsaverage5 --cifti-output"
      # --use-aroma"
